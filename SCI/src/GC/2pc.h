...
class C2PC { public:
	...
	const block128 MASK = makeBlock128(0x0ULL, 0xFFFFFULL);
	Fpre<T>* fpre = nullptr;
	block128 * mac = nullptr;
	block128 * key = nullptr;

	block128 * preprocess_mac = nullptr;
	block128 * preprocess_key = nullptr;

	block128 * sigma_mac = nullptr;
	block128 * sigma_key = nullptr;

	block128 * labels = nullptr;
	...
	C2PC(T * io, int party, BristolFormat * cf) {
		...
		key = new block128[cf->num_wire];
		mac = new block128[cf->num_wire];

		preprocess_mac = new block128[total_pre];
		preprocess_key = new block128[total_pre];

		//sigma values in the paper
		sigma_mac = new block128[num_ands];
		sigma_key = new block128[num_ands];

		labels = new block128[cf->num_wire];
	...
	block128 (* GTM)[4] = nullptr;

	//not allocation
	block128 * ANDS_mac = nullptr;
	block128 * ANDS_key = nullptr;
	void function_independent() {
		...
		memcpy(key, preprocess_key, (cf->n1+cf->n2)*sizeof(block128));
		memcpy(mac, preprocess_mac, (cf->n1+cf->n2)*sizeof(block128));
	}
	...
	void function_dependent() {
		...
		fpre->MAC = nullptr;
		fpre->KEY = nullptr;
		GT = new block128[num_ands][4][2];
		GTK = new block128[num_ands][4];
		GTM = new block128[num_ands][4];
		...
		for(int i = 0; i < cf->num_gate; ++i) {
			if(cf->gates[4*i+3] == AND_GATE) {
				...
				if(party == ALICE) {
					...
					memcpy(GTK[ands], K, sizeof(block128)*4);
					memcpy(GTM[ands], M, sizeof(block128)*4);
#ifdef __debug
					for(int j = 0; j < 4; ++j)
						check2(M[j], K[j]);
#endif
		...
		if(party == ALICE) {
			...
			for(int i = cf->n1; i < cf->n1+cf->n2; ++i) {
				...
				block128 ttt = key[i] ^ fpre->Delta;
				ttt =  ttt & MASK;
				block128 mask_key = key[i] & MASK;
				tmp =  tmp & MASK;
	...
	void online (const bool * input, bool * output, bool alice_output = false) {
		...
		if(party == BOB) {
			for(int i = 0; i < cf->num_gate; ++i) {
				if (cf->gates[4*i+3] == XOR_GATE) {
					...
					block128 ttt = GTK[ands][index] ^ fpre->Delta;
					ttt =  ttt & MASK;
		...
		if (party == BOB) {
			...
			for(int i = 0; i < cf->n3; ++i) {
				...
				block128 ttt = key[cf->num_wire - cf-> n3 + i] ^ fpre->Delta;
				ttt =  ttt & MASK;
			...
			if(alice_output) {
				block128 * tmp_mac = new block128[cf->n3];
				block128 * tmp_label = new block128[cf->n3];
				...
				for(int i = 0; i < cf->n3; ++i) {
					...
					block128 mask_label = tmp_label[i];
					if(tmp_mask_input[i])
						mask_label = mask_label ^ fpre->Delta;
					mask_label = mask_label & MASK;
					block128 masked_labels = labels[cf->num_wire - cf-> n3 + i] & MASK;
	...
	void check(block128 * MAC, block128 * KEY, bool * r, int length = 1) {
		if (party == ALICE) {
			io->send_data(r, length*3);
			io->send_block(&fpre->Delta, 1);
			io->send_block(KEY, length*3);
			block128 DD;io->recv_block(&DD, 1);

			for(int i = 0; i < length*3; ++i) {
				block128 tmp;io->recv_block(&tmp, 1);
				if(r[i]) tmp = tmp ^ DD;
	...
	void check2(block128 & MAC, block128 & KEY) {
		if (party == ALICE) {
			io->send_block(&fpre->Delta, 1);
			io->send_block(&KEY, 1);
			block128 DD;io->recv_block(&DD, 1);
	...
	void Hash(block128 H[4][2], const block128 & a, const block128 & b, uint64_t i) {
		block128 A[2], B[2];
		...
		for(uint64_t j = 0; j < 4; ++j) {
			H[j][0] = H[j][0] ^ makeBlock128(4*i+j, 0);
			H[j][1] = H[j][1] ^ makeBlock128(4*i+j, 1);
		}
		prp.permute_block((block128 *)H, 8);
	}

	void Hash(block128 H[2], block128 a, block128 b, uint64_t i, uint64_t row) {
		a = sigma(a);
		b = sigma(sigma(b));
		H[0] = H[1] = a ^ b;
		H[0] = H[0] ^ makeBlock128(4*i+row, 0);
		H[1] = H[1] ^ makeBlock128(4*i+row, 1);
		prp.permute_block((block128 *)H, 2);
	}
