...
class AmortizedC2PC { public:
	...
	block128 * preprocess_mac;
	block128 * preprocess_key;

	block128 * sigma_mac[exec];
	block128 * sigma_key[exec];

	block128 * labels[exec];
	...
	AmortizedC2PC(T * io, int party, BristolFormat * cf) {
		...
		preprocess_mac = new block128[total_pre*exec];
		preprocess_key = new block128[total_pre*exec];
		...
		for(int i = 0; i < exec; ++i) {
			...
			key[i] = new block128[cf->num_wire];
			mac[i] = new block128[cf->num_wire];

			//sigma values in the paper
			sigma_mac[i] = new block128[num_ands];
			sigma_key[i] = new block128[num_ands];

			labels[i] = new block128[cf->num_wire];
			GT[i] = new block128[num_ands][4][2];
			GTK[i] = new block128[num_ands][4];
			GTM[i] = new block128[num_ands][4];
	...
	PRG prg;
	PRP prp;
	block128 (* GT[exec])[4][2];
	block128 (* GTK[exec])[4];
	block128 (* GTM[exec])[4];

	//not allocation
	block128 * ANDS_mac[exec];
	block128 * ANDS_key[exec];
	void function_independent() {
		...
		for(int i = 0; i < exec; ++i) {
			memcpy(key[i], preprocess_key + total_pre * i, (cf->n1+cf->n2)*sizeof(block128));
			memcpy(mac[i], preprocess_mac + total_pre * i, (cf->n1+cf->n2)*sizeof(block128));
		}
	}
				...
				if (cf->gates[4*i+3] == AND_GATE) {
					...
					if(x1[e][ands] and y1[e][ands]) {
						if(party == ALICE) {
							...
							sigma_key[e][ands] = sigma_key[e][ands] ^ makeBlock128(0,1);
						}
						else
							sigma_mac[e][ands] = sigma_mac[e][ands] ^ makeBlock128(0,1);
					}
#ifdef __debug
					block128 MM[] = {mac[e][cf->gates[4*i]], mac[e][cf->gates[4*i+1]], sigma_mac[e][ands]};
					block128 KK[] = {key[e][cf->gates[4*i]], key[e][cf->gates[4*i+1]], sigma_key[e][ands]};
			...
			for(int i = 0; i < cf->num_gate; ++i) {
				if(cf->gates[4*i+3] == AND_GATE) {
					...
					if(party == ALICE) {
						...
						memcpy(GTK[e][ands], K, sizeof(block128)*4);
						memcpy(GTM[e][ands], M, sizeof(block128)*4);
#ifdef __debug
						for(int j = 0; j < 4; ++j)
							check2(M[j], K[j], r[j]);
#endif
		...
		if(party == ALICE) {
			...
			for(int e = 0; e < exec; ++e)
				for(int i = cf->n1; i < cf->n1+cf->n2; ++i) {
					...
					block128 ttt = key[e][i] ^ fpre->Delta;
					ttt =  ttt & MASK;
					block128 mask_key = key[e][i] & MASK;
					tmp =  tmp & MASK;
	...
	void online (bool * input, bool * output) {
		...
		if(party == BOB) {
			for(int i = 0; i < cf->num_gate; ++i) {
				if (cf->gates[4*i+3] == XOR_GATE) {
					...
					block128 ttt = GTK[exec_times][ands][index] ^ fpre->Delta;
					ttt =  ttt & MASK;
		...
		if (party == BOB) {
			...
			for(int i = 0; i < cf->n3; ++i) {
				...
				block128 ttt = key[exec_times][cf->num_wire - cf-> n3 + i] ^ fpre->Delta;
				ttt =  ttt & MASK;
