...
template<typename T>
void execute_circuit(block128 * wires, const T * gates, size_t num_gate) {
	for(size_t i = 0; i < num_gate; ++i) {
		if(gates[4*i+3] == AND_GATE) {
			...
			block128 tmp = circ_exec->xor_gate(wires[gates[4*i]],  wires[gates[4*i+1]]);
			block128 tmp2 = circ_exec->and_gate(wires[gates[4*i]], wires[gates[4*i+1]]);
			wires[gates[4*i+2]] = circ_exec->xor_gate(tmp, tmp2);
		}
	}
}
...
class BristolFormat { public:
	...
	void compute(Bit * out, const Bit * in1, const Bit * in2) {
		compute((block128*)out, (block128 *)in1, (block128*)in2);
	}

	void compute(block128 * out, const block128 * in1, const block128 * in2) {
		memcpy(wires.data(), in1, n1*sizeof(block128));
		memcpy(wires.data()+n1, in2, n2*sizeof(block128));
		for(int i = 0; i < num_gate; ++i) {
			if(gates[4*i+3] == AND_GATE) {
				wires[gates[4*i+2]] = circ_exec->and_gate(wires[gates[4*i]], wires[gates[4*i+1]]);
			}
		...
		memcpy(out, wires.data()+(num_wire-n3), n3*sizeof(block128));
	}
};
...
class BristolFashion { public:
	...
	void compute(Bit * out, const Bit * in) {
		compute((block128*)out, (block128 *)in);
	}
	void compute(block128 * out, const block128 * in) {
		memcpy(wires.data(), in, num_input*sizeof(block128));
		for(int i = 0; i < num_gate; ++i) {
			if(gates[4*i+3] == AND_GATE) {
				wires[gates[4*i+2]] = circ_exec->and_gate(wires[gates[4*i]], wires[gates[4*i+1]]);
			}
		...
		memcpy(out, wires.data()+(num_wire-num_output), num_output*sizeof(block128));
	}
};
