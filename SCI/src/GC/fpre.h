...
class Fpre {
	public:
		...
		block128 one;
		Feq<T> *eq[THDS*2];
		block128 * MAC = nullptr, *KEY = nullptr;
		block128 * MAC_res = nullptr, *KEY_res = nullptr;
		block128 * pretable = nullptr;
		Fpre(T * in_io, int in_party, int bsize = 1000) {
			...
			one = makeBlock128(0, 1);
			ZDelta =  Delta  & makeBlock128(0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFE);
		...
		void set_batch_size(int size) {
			...
			MAC = new block128[batch_size * bucket_size * 3];
			KEY = new block128[batch_size * bucket_size * 3];
			MAC_res = new block128[batch_size * 3];
			KEY_res = new block128[batch_size * 3];
//			cout << size<<"\t"<<batch_size<<"\n";
...
#endif
			block128 S = coin_tossing(prg, io[0], party);
			if(bucket_size > 4) {
		...
		void generate(block128 * MAC, block128 * KEY, int length, int I) {
			if (party == ALICE) {
		...
		void check(block128 * MAC, block128 * KEY, int length, int I) {
			T * local_io = (I%2==0) ? io[I/2]: io2[I/2];

			block128 * G = new block128[length];
			block128 * C = new block128[length];
			block128 * GR = new block128[length];
			...
			if(party == ALICE) {
				local_io->send_data(G, sizeof(block128)*length);
				local_io->recv_data(GR, sizeof(block128)*length);
			} else {
				local_io->recv_data(GR, sizeof(block128)*length);
				local_io->send_data(G, sizeof(block128)*length);
			}
			local_io->flush();
			for(int i = 0; i < length; ++i) {
				block128 S = H2(MAC[3*i], KEY[3*i], I);
		...
		block128 H2(block128 a, block128 b, int I) {
			block128 d[2];
			d[0] = a;
			d[1] = b;
			prps[I].permute_block(d, 2);
...
//for debug
		void check_correctness(block128 * MAC, block128 * KEY, int length) {
			if (party == ALICE) {
				...
				block128 DD;io[0]->recv_block(&DD, 1);

				for(int i = 0; i < length*3; ++i) {
					block128 tmp;io[0]->recv_block(&tmp, 1);
